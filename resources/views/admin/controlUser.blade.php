...
<div class="container">
        ...
        <div class="row align-items-center">
            <div class="col-md-6">
                ...
                <p class="text-muted mb-0">
                    ...
                    {{ $data->total() }} utilisateurs au total
                </p>
            </div>
    ...
    <div class="card user-card">
        <div class="card-body">
            <form id="searchForm" method="GET" action="{{ url('admin/controluser') }}">
                    ...
                    <div class="row align-items-center">
                        ...
                        <div class="col-md-3">
                            <select name="role" class="form-control">
                                ...
                                <option value="pending" {{ request('role') == 'pending' ? 'selected' : '' }}>En attente</option>
                                <option value="1" {{ request('role') == '1' ? 'selected' : '' }}>Administrateur</option>
                                <option value="2" {{ request('role') == '2' ? 'selected' : '' }}>Enseignant</option>
                                <option value="3" {{ request('role') == '3' ? 'selected' : '' }}>Étudiant</option>
                            </select>
                        </div>
                        <div class="col-md-6">
                            ...
                            <a href="{{ url('admin/controluser') }}" class="btn btn-secondary ml-2">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                @else
                                <span class="role-badge role-{{ $user->category }}">
                                    {{ $user->category == 1 ? 'Admin' : ($user->category == 2 ? 'Enseignant' : 'Étudiant') }}
                                </span>
                                @endif
                            </td>
                            <td>
                            ...
                            <td>
                                <div class="action-buttons">
                                    <button class="btn btn-icon btn-edit"
                                        onclick="openEditModal('{{ $user->id }}', '{{ $user->name }}', '{{ $user->email }}', '{{ $user->category }}')"
                                        title="Modifier">
...
<script>
    ...
    function openEditModal(userId, name, email, category) {
        document.getElementById('editUserForm').action = `{{ url('admin/updateGat') }}/${userId}`;
    ...
    function showDeleteConfirmation(userId, userName) {
        document.getElementById('confirmDeleteButton').href = "{{ url('admin/deleteuser') }}/" + userId;
