...
<div class="container">
    ...
    <div class="content-grid">
        ...
        <div class="content-card">
            <div class="content-card-header {{ $item->catg == 'course' ? 'bg-primary' : ($item->catg == 'exam' ? 'bg-info' : 'bg-success') }}">
                <div class="content-icon">
                    <i class="fas {{ $item->catg == 'course' ? 'fa-book' : ($item->catg == 'exam' ? 'fa-file-alt' : 'fa-tasks') }}"></i>
                </div>
                <h3 class="content-title">{{ $item->name }}</h3>
                <span class="content-type">
                    {{ $item->catg == 'course' ? 'Cours' : ($item->catg == 'exam' ? 'Examen' : 'Test') }}
                </span>
            </div>
            <div class="content-card-body">
                <p class="content-description">{{ $item->description }}</p>
                <div class="content-meta">
                    <span><i class="far fa-clock mr-1"></i>{{ \Carbon\Carbon::parse($item->created_at)->format('d/m/Y') }}</span>
                </div>
                <div class="content-actions">
                    @if($item->catg == 'course')
                        <a href="{{ url('teacher/mesCours/pdf/'.$item->id) }}" class="btn btn-sm btn-outline-primary">
                            <i class="fas fa-file-pdf mr-1"></i>PDF
                        </a>
                    @elseif($item->catg == 'exam')
                        <a href="{{ url('teacher/mesCours/viewExam/'.$item->id) }}" class="btn btn-sm btn-outline-info">
                            <i class="fas fa-eye mr-1"></i>Voir
                        </a>
                    @else
                        <a href="{{ url('teacher/mesCours/viewTest/'.$item->id) }}" class="btn btn-sm btn-outline-success">
                            <i class="fas fa-eye mr-1"></i>Voir
                        </a>
                    @endif
                </div>
...
<div class="modal fade" id="addCourseModal">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <form action="{{ url('teacher/mesCours/addCours/'.$id) }}" method="post">
                @csrf
...
<div class="modal fade" id="addExamModal">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form action="{{ url('teacher/mesCours/addExam/'.$id) }}" method="post">
                @csrf
                <div class="modal-body">
                    <div class="form-group">
...
<div class="modal fade" id="addTestModal">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form action="{{ url('teacher/mesCours/addTest/'.$id) }}" method="post">
                @csrf
                <div class="modal-body">
                    <div class="form-group">
